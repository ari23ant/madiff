...
//noindent
帯状の波形に見えるが、これだと見通しが悪いので、今回は最初の10秒のみを扱うこととする。
...
//noindent
心電図らしい波形が見えてきたので、これでデータが準備できた。以降で、この波形から心拍数を求めてみる。


== 心拍数算出
さて、分析を始めてみよう。まず、想像する心電図と違い大きく揺らいでいるので、移動平均フィルタによるハイパスフィルタで基線変動を除去する。
...
//noindent
橙線を見ると、きれいに基線変動が除去できていることがわかる。このままでもよいが、高周波ノイズが重畳している。そこで、再び移動平均フィルタを使って、高周波ノイズを除去する。
...
きれいにR波を検出できたので、心拍数を求めよう。心拍数の単位は[bpm]（beats per minuteの略）であるため、1分間に何回拍動するかを計算すればよい。
...
上記はいずれも答えがあるものではなく、読者の理解を深めるために設けた。ぜひ自分の手で試してみてほしい。
