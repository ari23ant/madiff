== 移動平均フィルタの応用
一般に、基線変動は取得したい波形よりも低周波である。したがって、移動平均フィルタを使ったハイパスフィルタで除去することが可能である。ここでは以下の波形を考えてみる。

//texequation[sinwavebaseline]{
x(t) = A_{1} \sin 2 \pi f_{1} \ t + a t
//}

第1項が抽出したい正弦波で、第2項が重畳した基線変動である。簡単のため基線変動は直線とした。この基線変動を、移動平均フィルタを使ったハイパスフィルタで除去し、正弦波を抽出する。設定値は以下の通り。
...
//image[04rmbaselinema][正弦波に基線変動が重畳した波形（青）と移動平均フィルタによるハイパスフィルタで基線変動を除去した波形（橙）][scale=0.8]

//noindent
@<list>{04rmbaselinema.py}で作ったフィルタは、前述のハイパスフィルタと同じである。@<img>{04rmbaselinema}より、移動平均フィルタで作ったハイパスフィルタで基線変動が除去できていることがわかる。
