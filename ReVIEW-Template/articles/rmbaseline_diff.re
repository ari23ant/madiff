== 微分フィルタの応用
実は微分フィルタでも基線変動が除去できる。上述と同じように@<m>$x(t) = A_{1} \sin 2 \pi f_{1} \ t + a t$を題材にして考えてみる。簡単のため@<m>$\omega_{1} = 2\pi f_{1}$とし、時間@<m>{t}で微分する。
...
//image[05rmbaselinediff][正弦波に基線変動が重畳した波形（青）と微分フィルタによるハイパスフィルタで基線変動を除去した波形（橙）][scale=0.8]

//noindent
@<img>{05rmbaselinediff}より、微分フィルタで基線変動が除去できていることがわかる。

ところで、元の波形（青）と基線変動を除去した波形（橙）を比べると、後者の方が振幅が大きい。これは@<eq>{xt_dxdt}のように、微分後は微分前と比べて振幅が@<m>$\omega_{1}$倍になるためとわかる。
...
以下の2つを考えてもらいたい。
 ...
 1. 微分フィルタを通すと位相がズレるが、正弦波であるとき具体的にどのくらいズレるか
